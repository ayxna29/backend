...
app.post('/generate_flashcards', async (req: TraceRequest, res: Response) => {
  ...
  let reusedGenerationRecord = false;
  ...
  try {
    ...
    const rawInput: any = req.body || {};
    const body = parseGenerateBody(rawInput, DEFAULT_COUNT);
    ...
    console.log('[GEN CLEANED REVISED]', { gen: generationId, input: cleanedStats.input, kept: cleanedStats.kept, rejected_dup_simple: cleanedStats.rejected_dup_simple });
    ...
    if (ENABLE_EMBED && embedRequested && keptCards.length) {
      try {
        const existingEmbeddings = await fetchExistingEmbeddings(user.id);
    ...
    const forcedAnswerLength = 'short';
    ...
    console.log('[POST-CLAMP SHORT]', { gen: generationId, samples: keptCards.slice(0, 3).map(c => c.answer) });

    let finalTagsSet = new Set(incomingTags);
    if (inferTagsFlag) {
      const inferred = inferTagsFromCards(keptCards).filter(t => /^[a-z0-9._-]{3,40}$/.test(t));
    ...
    if (finalTags.length && inserted?.length) {
      ...
      if (tagRows.length) {
        const { error: tagErr } = await getSupabase().from('flashcard_tags').insert(tagRows);
        if (tagErr) console.error('[tag insert error]', tagErr);
      }
    }

    try {
      await logGenerationComplete({ generationId: generationId ?? '', cards: keptCards, raw: null, latencyMs: Date.now() - t0, modelName: modelUsed });
    } catch (e) {
    ...
    return res.json({
      ...
      reused: false,
      reused_generation: reusedGenerationRecord,
    ...
    if (generationId) {
      try {
        await logGenerationComplete({ generationId, cards: [], raw: null, latencyMs: Date.now() - t0, modelName: modelUsed, error: e?.message || 'error' });
      } catch {}
    }
...
app.post('/flashcards/:id/favorite', async (req, res) => {
  ...
  try {
    ...
    const { error: fErr } = await sb.from('flashcard_favorites').upsert([{ user_id: user.id, card_id: cardId }], { onConflict: 'user_id,card_id' });
    if (fErr) throw fErr;
...
app.delete('/flashcards/:id/favorite', async (req, res) => {
  ...
  try {
    const { error } = await getSupabase().from('flashcard_favorites').delete().eq('user_id', user.id).eq('card_id', cardId);
...
app.get('/optimization/tags', async (req, res) => {
  ...
  if (!user) return res.status(401).json({ error: 'unauthorized' });
  try {
    const { data: tags, error } = await getSupabase().from('user_tags').select('id, tag_name, created_at').eq('user_id', user.id).order('created_at', { ascending: true });
    if (error) throw error;
    res.json({ tags: tags || [] });
  } catch (e) {
...
async function createDefaultFavoritesForUser(sb: any, user: { id: string; email?: string | null }) {
  ...
  for (const word of defaults) {
    const { data: existingCards } = await sb.from('flashcards').select('id, asset_filename').eq('user_id', user.id).ilike('answer', word).limit(1);
    ...
    if (existingCards && existingCards.length) {
      ...
      const { error: genUpsertErr } = await sb.from('flashcard_generations').upsert([{
        id: generationId, user_id: user.id, context_hash: ctxHash, context_text: word,
        model_name: 'builtin-defaults', prompt_version: 1, created_at: new Date().toISOString()
      }], { onConflict: 'id' });
      if (genUpsertErr) { console.error('[SEED DEFAULTS] generation upsert failed', genUpsertErr); throw genUpsertErr; }

      const { data: inserted, error: insertErr } = await sb.from('flashcards')
        .insert([{ generation_id: generationId, user_id: user.id, question: word, answer: word, asset_filename: symbol, tag: null, created_at: new Date().toISOString() }])
        .select('id, asset_filename').limit(1);
      if (insertErr) { console.error('[SEED DEFAULTS] insert flashcard error', insertErr); throw insertErr; }
      else if (inserted && inserted.length) { cardId = inserted[0].id; assetFilename = inserted[0].asset_filename || null; }
    }

    if (cardId) {
      const { error: favErr } = await sb.from('flashcard_favorites').upsert([{ user_id: user.id, card_id: cardId }], { onConflict: 'user_id,card_id' });
      if (!favErr) createdFavorites.push({ card_id: cardId, answer: word, asset_filename: assetFilename });
    }
  }
...
app.post('/optimization/tags', async (req, res) => {
  ...
  try {
    const { data, error } = await getSupabase().from('user_tags').insert([{ user_id: user.id, tag_name: tag_name.trim() }]).select().single();
    if (error) throw error;
    res.json({ tag: data });
  } catch (e: any) {
    if (e?.code === '23505') return res.status(409).json({ error: 'tag_already_exists' });
...
app.delete('/optimization/tags/:id', async (req, res) => {
  ...
  try {
    const { data: tag } = await getSupabase().from('user_tags').select('id').eq('id', tagId).eq('user_id', user.id).single();
    if (!tag) return res.status(404).json({ error: 'tag_not_found' });
    const { error } = await getSupabase().from('user_tags').delete().eq('id', tagId);
    if (error) throw error;
    res.json({ ok: true });
  } catch (e) {
...
app.get('/optimization/tags/:tagId/contexts', async (req, res) => {
  ...
  try {
    const { data: tag } = await getSupabase().from('user_tags').select('id').eq('id', tagId).eq('user_id', user.id).single();
    if (!tag) return res.status(404).json({ error: 'tag_not_found' });
    const { data: contexts, error } = await getSupabase().from('tag_contexts').select('id, context_text, created_at').eq('tag_id', tagId).order('created_at', { ascending: true });
    if (error) throw error;
...
app.post('/optimization/tags/:tagId/contexts', async (req, res) => {
  ...
  if (!context_text || typeof context_text !== 'string') return res.status(400).json({ error: 'context_text required' });
  try {
    const { data: tag } = await getSupabase().from('user_tags').select('id').eq('id', tagId).eq('user_id', user.id).single();
    if (!tag) return res.status(404).json({ error: 'tag_not_found' });
    const { data, error } = await getSupabase().from('tag_contexts').insert([{ tag_id: tagId, context_text: context_text.trim() }]).select().single();
    if (error) throw error;
...
app.delete('/optimization/contexts/:id', async (req, res) => {
  ...
  try {
    const { data: context } = await getSupabase().from('tag_contexts').select('id, tag_id, user_tags!inner(user_id)').eq('id', contextId).single();
    if (!context || (context.user_tags as any).user_id !== user.id) return res.status(404).json({ error: 'context_not_found' });
    const { error } = await getSupabase().from('tag_contexts').delete().eq('id', contextId);
    if (error) throw error;
    res.json({ ok: true });
  } catch (e) {
...
app.patch('/optimization/contexts/:id', async (req, res) => {
  ...
  if (!context_text || typeof context_text !== 'string') return res.status(400).json({ error: 'context_text required' });
  try {
    const { data: ctxRow, error: ctxErr } = await getSupabase().from('tag_contexts').select('id, tag_id').eq('id', contextId).limit(1).maybeSingle();
    if (ctxErr) throw ctxErr;
    if (!ctxRow) return res.status(404).json({ error: 'context_not_found' });
    const { data: tagRow, error: tagErr } = await getSupabase().from('user_tags').select('id, user_id').eq('id', ctxRow.tag_id).limit(1).maybeSingle();
    if (tagErr) throw tagErr;
    if (!tagRow || tagRow.user_id !== user.id) return res.status(404).json({ error: 'context_not_found' });
    const { data, error } = await getSupabase().from('tag_contexts').update({ context_text: context_text.trim() }).eq('id', contextId).select().single();
    if (error) throw error;
...
app.get('/optimization/favorites', async (req, res) => {
  ...
  try {
    const sb = getSupabase();
    const { data: optFavs = [], error: optErr } = await sb.from('user_favorites').select('id, word, asset_filename, created_at').eq('user_id', user.id).order('created_at', { ascending: true });
    if (optErr) throw optErr;
    const { data: canonFavs = [], error: canonErr } = await sb.from('flashcard_favorites').select('card_id, created_at, flashcards(id, answer, asset_filename)').eq('user_id', user.id).order('created_at', { ascending: true });
    if (canonErr) throw canonErr;

    const canonMapped = (canonFavs || []).map((r: any) => {
      const card = r.flashcards || r.flashcard || {};
      return { id: `card_${r.card_id}`, word: (card.answer || '').toString(), asset_filename: card.asset_filename || null, created_at: r.created_at };
    });
...
app.post('/optimization/favorites', async (req, res) => {
  ...
  try {
    const sb = getSupabase();
    const { data: ufData, error: ufErr } = await sb.from('user_favorites').insert([{ user_id: user.id, word: word.trim(), asset_filename: asset_filename || null }]).select().maybeSingle();
    if (ufErr && ufErr.code !== '23505') throw ufErr;

    const trimmed = word.trim();
    const { data: existingCards } = await sb.from('flashcards').select('id, asset_filename').eq('user_id', user.id).ilike('answer', trimmed).limit(1);
    let cardId: string | null = null;
    ...
    if (existingCards && existingCards.length) {
      ...
      const { error: genUpsertErr2 } = await sb.from('flashcard_generations').upsert([{ id: generationId, user_id: user.id, context_hash: ctxHash, context_text: trimmed, model_name: 'optimization-fav', prompt_version: 1, created_at: new Date().toISOString() }], { onConflict: 'id' });
      if (genUpsertErr2) throw genUpsertErr2;
      const { data: inserted, error: insertErr } = await sb.from('flashcards').insert([{ generation_id: generationId, user_id: user.id, question: trimmed, answer: trimmed, asset_filename: symbol, tag: null, created_at: new Date().toISOString() }]).select('id, asset_filename').limit(1);
      if (insertErr) throw insertErr;
      else if (inserted && inserted.length) { cardId = inserted[0].id; chosenAsset = inserted[0].asset_filename || chosenAsset; }
    }
    ...
    try {
      const { data: optFavs = [] } = await sb.from('user_favorites').select('id, word, asset_filename, created_at').eq('user_id', user.id).order('created_at', { ascending: true });
      const { data: canonFavs = [] } = await sb.from('flashcard_favorites').select('card_id, created_at, flashcards(id, answer, asset_filename)').eq('user_id', user.id).order('created_at', { ascending: true });
      const canonMapped = (canonFavs || []).map((r: any) => { const card = r.flashcards || r.flashcard || {}; return { id: `${card.id}`, word: (card.answer || '').toString(), asset_filename: card.asset_filename || null, created_at: r.created_at }; });
      const merged: any[] = [];
      const seen = new Set<string>();
      const pushIfNew = (item: any) => { const key = (item.word || '').toString().trim().toLowerCase(); if (!key || seen.has(key)) return; seen.add(key); merged.push(item); };
      for (const c of canonMapped) pushIfNew(c);
      for (const o of (optFavs || [])) pushIfNew(o);
      merged.sort((a: any, b: any) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
      return res.json({ optimization_favorite: ufData || null, flashcard_id: cardId, asset_filename: chosenAsset || 'blank.svg', favorites: merged });
    } catch (e) {
      return res.json({ optimization_favorite: ufData || null, flashcard_id: cardId, asset_filename: chosenAsset || 'blank.svg' });
    }
  } catch (e: any) {
    if (e?.code === '23505') return res.status(409).json({ error: 'favorite_already_exists' });
...
app.delete('/optimization/favorites/:id', async (req, res) => {
  ...
  try {
    if (favoriteId.startsWith('card_')) {
      ...
      const { error } = await getSupabase().from('flashcard_favorites').delete().eq('card_id', cardId).eq('user_id', user.id);
      if (error) throw error;
    } else {
      const { error } = await getSupabase().from('user_favorites').delete().eq('id', favoriteId).eq('user_id', user.id);
      if (error) throw error;
    }
    res.json({ ok: true });
  } catch (e) {
...
function matchSymbolFilename(answer: string, availableSymbols: string[]): string {
  ...
  const synonymMap: Record<string, string> = {
    ...
    'angry': 'angry_lady', 'mad': 'angry_lady', 'frustrated': 'angry_lady', 'cross': 'angry_lady',
    'tired': 'yawn_,_to', 'exhausted': 'yawn_,_to', 'sleepy': 'yawn_,_to', 'drowsy': 'yawn_,_to',
    'sick': 'vomit_,_to', 'ill': 'vomit_,_to', 'unwell': 'vomit_,_to',
    'scared': 'afraid_lady', 'afraid': 'afraid_lady', 'frightened': 'afraid_lady', 'terrified': 'afraid_lady',
    'eat': 'eat_,_to', 'eating': 'eat_,_to', 'drink': 'drink_,_to', 'drinking': 'drink_,_to',
    'help': 'help_,_to', 'want': 'want_,_to', 'go': 'go_through_door_,_to',
    'run': 'run_,_to', 'running': 'run_,_to', 'wait': 'wait_,_to', 'waiting': 'wait_,_to',
    'sleep': 'sleep_female_,_to', 'sleeping': 'sleep_female_,_to', 'nap': 'sleep_female_,_to',
    'play': 'play_area', 'playing': 'play_area',
    'finish': 'finish', 'done': 'finish', 'finished': 'finish', 'complete': 'finish',
    'yes': 'correct', 'yeah': 'correct', 'yep': 'correct', 'ok': 'correct', 'okay': 'correct',
    'no': 'mistake_no_wrong', 'nope': 'mistake_no_wrong', 'nah': 'mistake_no_wrong',
    'more': 'more', 'again': 'more',
    'cold': 'drink_cold', 'cool': 'drink_cold', 'hot': 'drink_hot', 'warm': 'drink_hot',
    'i': 'I', 'mom': 'mum_parent', 'mum': 'mum_parent', 'mother': 'mum_parent', 'mommy': 'mum_parent', 'mama': 'mum_parent',
    'dad': 'dad_parent', 'father': 'dad_parent', 'daddy': 'dad_parent', 'papa': 'dad_parent',
    'home': 'motor_home', 'house': 'motor_home', 'school': 'school',
    'water': 'water', 'food': 'cat_food', 'meal': 'cat_food',
    'breakfast': 'breakfast', 'lunch': 'lunch', 'dinner': 'dinner', 'snack': 'snack',
    'hungry': 'hungry', 'thirsty': 'water',
    'pancake': 'pancake', 'pancakes': 'pancake', 'sandwich': 'sandwich', 'pizza': 'pizza',
    'apple': 'apple', 'cereal': 'cereal', 'bread': 'bread', 'burger': 'burger',
  ...
  if (normalized.includes('_')) {
    ...
    for (const t of tokens) {
      const exactTok = availableSymbols.find(s => { const base = s.toLowerCase().replace('.svg', ''); return base === t || s.toLowerCase() === `${t}.svg`; });
      if (exactTok) { console.log('[TOKEN EXACT MATCH]', { token: t, matched: exactTok }); return exactTok; }
      ...
      if (tokSyn) {
        ...
        if (synMatch) { console.log('[TOKEN SYNONYM MATCH]', { token: t, syn: tokSyn, matched: synMatch }); return synMatch; }
      }
    }
  }
  ...
  if (!normalized.includes('_')) {
    ...
    if (tokenList && tokenList.length) {
      ...
      if (exactToken) { console.log('[TOKEN MAP EXACT]', { answer, matched: exactToken }); return exactToken; }
      const pref = tokenList.find(s => s.toLowerCase().split('_')[0] === normalized);
      if (pref) { console.log('[TOKEN MAP PREFIX]', { answer, matched: pref }); return pref; }
    ...
    if (normalized.length < 4) {
      ...
      if (shortSyn) {
        const shortMatch = availableSymbols.find(s => s.toLowerCase() === `${shortSyn}.svg` || s.toLowerCase().startsWith(`${shortSyn}_`));
        if (shortMatch) { console.log('[SHORT SYNONYM MATCH]', { answer, synonym: shortSyn, matched: shortMatch }); return shortMatch; }
      }
  ...
  for (const s of availableSymbols) {
    ...
    const parts = base.split('_').map(p => p.trim()).filter(Boolean);
    for (const p of parts) {
      if (p === normalized) { containsCandidates.push(s); break; }
      if (p.startsWith(normalized) && Math.abs(p.length - normalized.length) <= 3 && normalized.length >= 3) { containsCandidates.push(s); break; }
      if (p.includes(normalized) && Math.abs(p.length - normalized.length) <= 2 && normalized.length >= 4) { containsCandidates.push(s); break; }
    }
  }
